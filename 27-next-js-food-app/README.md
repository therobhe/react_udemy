...
- `useFormState` hook from react-dom is used for mirroring server side validated errors on demand without needing to
  redirect to an error page and maintain the current inputted data
- `useFormStatus` can detect if a form is pending and is submitting thus its a great tool for showing a loading spinner
